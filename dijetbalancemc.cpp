...
void dijetbalancemc()	{
  ...
  for (int radius_counter = 0; radius_counter < number_of_radii; radius_counter++) { //RADIUS LOOP
	...
	if (pt->IsOpen()) pt->ls();
	TFile *balancemcout = new TFile(Form("ak%iPFJetAnalyzer/balancemcout123.root",radius), "recreate"); // 60 TO 80 PT AVERAGE
	...
	if (pt->IsOpen()) pt->ls();
	TFile *balancemcout = new TFile(Form("ak%iPFJetAnalyzer/balancemcouta.root",radius), "recreate"); // 80 to 100 PT AVERAGE
	cout << "File 3 Opened Successfully" << endl;
      }
      if (filecounter == 3) {
	TFile *pt = TFile::Open("/afs/cern.ch/user/g/gpikul/work/public/pt80_pp2013_P01_prod22_v81_merged_forest_0.root"); // 80 TO 140 PT AVERAGE
	if (pt->IsOpen()) pt->ls();
	TFile *balancemcout = new TFile(Form("ak%iPFJetAnalyzer/balancemcout.root",radius), "recreate"); // 100 TO 140 PT AVERAGE
	...
	if (pt->IsOpen()) pt->ls();
	TFile *balancemcout = new TFile(Form("ak%iPFJetAnalyzer/balancemcout12.root",radius), "recreate"); // 140 TO 200 PT AVERAGE
