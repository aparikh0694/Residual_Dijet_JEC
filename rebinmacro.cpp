...
void rebinmacro() {
  ...
  for (int radius_counter = 1; radius_counter <= 7; radius_counter++) { //RADIUS LOOP
    ...
    for (int filecounter = 0; filecounter < 5; filecounter++) { //FILE LOOP
      if (filecounter == 0) {
	TFile *MyFile = new TFile(Form("ak%iPFJetAnalyzer/balanceout1234.root",radius_counter), "Read"); // 40 TO 60 PT AVERAGE
	...
	TFile *rebinfile = new TFile(Form("ak%iPFJetAnalyzer/rebinout1234.root",radius_counter), "Recreate"); // 40 TO 60 PT AVERAGE
      }
      if (filecounter == 1) {
	TFile *MyFile = new TFile(Form("ak%iPFJetAnalyzer/balanceout123.root",radius_counter), "Read"); // 60 TO 80 PT AVERAGE
	if (MyFile->IsOpen()) printf("File Opened Successfully.\n");
	gFile = MyFile;
	MyFile->ls();
	TH1F *balance = (TH1F *)MyFile->Get("balance");
	TH1F *outereta = (TH1F *)MyFile->Get("outereta");
	TH1F *innereta = (TH1F *)MyFile->Get("innereta");
	TFile *rebinfile = new TFile(Form("ak%iPFJetAnalyzer/rebinout123.root",radius_counter), "Recreate"); // 60 TO 80 PT AVERAGE
      }
      if (filecounter == 2) {
	TFile *MyFile = new TFile(Form("ak%iPFJetAnalyzer/balanceout1.root",radius_counter), "Read"); // 80 TO 100 PT AVERAGE
	if (MyFile->IsOpen()) printf("File Opened Successfully.\n");
	gFile = MyFile;
	MyFile->ls();
	TH1F *balance = (TH1F *)MyFile->Get("balance");
	TH1F *outereta = (TH1F *)MyFile->Get("outereta");
	TH1F *innereta = (TH1F *)MyFile->Get("innereta");
	TFile *rebinfile = new TFile(Form("ak%iPFJetAnalyzer/rebinout1.root",radius_counter), "Recreate"); // 80 TO 100 PT AVERAGE
      }
      if (filecounter == 3) {
	TFile *MyFile = new TFile(Form("ak%iPFJetAnalyzer/balanceout.root",radius_counter), "Read"); // 100 TO 140 PT AVERAGE
	if (MyFile->IsOpen()) printf("File Opened Successfully.\n");
	gFile = MyFile;
	MyFile->ls();
	TH1F *balance = (TH1F *)MyFile->Get("balance");
	TH1F *outereta = (TH1F *)MyFile->Get("outereta");
	TH1F *innereta = (TH1F *)MyFile->Get("innereta");
	TFile *rebinfile = new TFile(Form("ak%iPFJetAnalyzer/rebinout.root",radius_counter), "Recreate"); // 100 TO 140 PT AVERAGE
      }
      if (filecounter == 4) {
	TFile *MyFile = new TFile(Form("ak%iPFJetAnalyzer/balanceout12.root",radius_counter), "Read"); // 140 TO 200 PT AVERAGE
	if (MyFile->IsOpen()) printf("File Opened Successfully.\n");
	gFile = MyFile;
	MyFile->ls();
	TH1F *balance = (TH1F *)MyFile->Get("balance");
	TH1F *outereta = (TH1F *)MyFile->Get("outereta");
	TH1F *innereta = (TH1F *)MyFile->Get("innereta");
	TFile *rebinfile = new TFile(Form("ak%iPFJetAnalyzer/rebinout12.root",radius_counter), "Recreate"); // 140 TO 200 PT AVERAGE
      }
	...
	//outeretarebin->Fill(integral2);
      ...
      balancerebin->SetTitle(Form("Rebin Full Eta Range Radius = 0.%i", radius_counter));
      ...
      inneretarebin->SetTitle(Form("Rebin Inner Eta Range Radius = 0.%i", radius_counter));
      ...
      outeretarebin->SetTitle(Form("Rebin Outer Eta Range Radius = 0.%i", radius_counter));
