...
void relativeresponsemc() {
  ...
  for (int radius_counter = 0; radius_counter < number_of_radii; radius_counter++) { //RADIUS LOOP
    ...
    for (int filecounter = 0; filecounter < 5; filecounter++) { //FILE LOOP
      if (filecounter == 0) {
	TFile *a = TFile::Open(Form("ak%iPFJetAnalyzer/merged1234.root",radius)); // 40 TO 60 PT AVERAGE
	TFile *relresp = new TFile(Form("ak%iPFJetAnalyzer/relativeresponsemc1234.root",radius), "Recreate"); // 40 TO 60 PT AVERAGE
	cout << "File 1 Opened Successfully" << endl;
      }
      if (filecounter == 1) {
	TFile *a = TFile::Open(Form("ak%iPFJetAnalyzer/merged123.root",radius)); // 60 TO 80 PT AVERAGE 
	TFile *relresp = new TFile(Form("ak%iPFJetAnalyzer/relativeresponsemc123.root",radius), "Recreate"); // 60 TO 80 PT AVERAGE
	cout << "File 2 Opened Successfully" << endl;
      }
      if (filecounter == 2) {
	TFile *a = TFile::Open(Form("ak%iPFJetAnalyzer/balancemcouta.root",radius)); //80 TO 100 PT AVERAGE
	TFile *relresp = new TFile(Form("ak%iPFJetAnalyzer/relativeresponsemc1.root",radius), "Recreate"); // 80 TO 100 PT AVERAGE
	cout << "File 3 Opened Successfully" << endl;
      }
      if (filecounter == 3) {
	TFile *a = TFile::Open(Form("ak%iPFJetAnalyzer/merged.root",radius)); // 100 TO 140 PT AVERAGE
	TFile *relresp = new TFile(Form("ak%iPFJetAnalyzer/relativeresponsemc.root",radius), "Recreate"); // 100 TO 140 PT AVERAGE
	cout << "File 4 Opened Successfully" << endl;
      }
      if (filecounter == 4) {
	TFile *a = TFile::Open(Form("ak%iPFJetAnalyzer/merged12.root",radius)); // 140 TO 200 PT AVERAGE
	TFile *relresp = new TFile(Form("ak%iPFJetAnalyzer/relativeresponsemc12.root",radius), "Recreate"); // 140 TO 200 PT AVERAGE
	...
	cout << binnum << endl;*/
      ...
      mytree->Draw("myb:myetaprobe>>b", "myweight", "");
