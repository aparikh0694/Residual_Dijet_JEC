...
void dijetbalancemc2()	{
  for (int filecounter = 0; filecounter < 4; filecounter++) { //FILE LOOP	
	...
	//OPEN FILE
    if (filecounter == 0) {
       TFile *pt = TFile::Open("/afs/cern.ch/user/g/gpikul/work/public/pt50_pp2013_P01_prod22_v81_merged_forest_0.root"); // 40 TO 60 PT AVERAGE
       if (pt->IsOpen()) pt->ls();
       TFile *balancemcout2 = new TFile("balancemcout21234.root", "recreate"); // 40 TO 60 PT AVERAGE
       cout << "File 1 Opened Successfully" << endl;
    }
    if (filecounter == 1) {
      TFile *pt = TFile::Open("/afs/cern.ch/user/g/gpikul/work/public/pt80_pp2013_P01_prod22_v81_merged_forest_0.root"); // 60 TO 80 PT AVERAGE 
      if (pt->IsOpen()) pt->ls();
      TFile *balancemcout2 = new TFile("balancemcout2123.root", "recreate"); // 60 TO 80 PT AVERAGE
      cout << "File 2 Opened Successfully" << endl;
    }
    if (filecounter == 2) {
      TFile *pt = TFile::Open("/afs/cern.ch/user/g/gpikul/work/public/pt120_pp2013_P01_prod22_v81_merged_forest_0.root"); // 80 TO 140 PT AVERAGE
      if (pt->IsOpen()) pt->ls();
      TFile *balancemcout2 = new TFile("balancemcout2.root", "recreate");  // 100 TO 140 PT AVERAGE
      cout << "File 3 Opened Successfully" << endl;
    }
    if (filecounter == 3) {
      TFile *pt = TFile::Open("/afs/cern.ch/user/g/gpikul/work/public/pt170_pp2013_P01_prod22_v81_merged_forest_0.root"); // 140 TO 200 PT AVERAGE
      if (pt->IsOpen()) pt->ls();
      TFile *balancemcout2 = new TFile("balancemcout212.root", "recreate"); // 140 TO 200 PT AVERAGE
	...
	//OPEN NECESSARY TREE
	TTree *ak3t = (TTree *)pt->Get("ak3PFJetAnalyzer/t");
  ...
  Float_t myetaref = 0;
  Float_t myb = 0;
  //Float_t myweight = 4107.57/9983.18; //TRIAL 1 EXCLUDING ANY JETS IN THE MC80 FILE THAT HAD PT ABOVE 120
  //Float_t myweight = 3458.4382/12522.7766; //TRIAL 2 INCLUDING JETS IN THE MC80 FILE THAT HAD PT ABOVE 120

  //Float_t myweight = 1; //TEST RUN
  //HAS TO CHANGE WITH EACH CONE SIZE
  if (filecounter == 0) Float_t myweight = (float)660/3094; // 40 TO 60 PT AVERAGE
  if (filecounter == 1) Float_t myweight = (float)441/1922; // 60 TO 80 PT AVERAGE
  if (filecounter == 2) Float_t myweight = (float)320/1473; //TRIAL 3 INCLUDING JETS IN THE MC80 FILE THAT HAD PT ABOVE 120 AND DONE USING THE BIN METHOD 80 TO 140 PT AVERAGE
  if (filecounter == 3) Float_t myweight = (float)443/1523; // 140 TO 200 PT AVERAGE
  Float_t myjtpt = 0;
...
//	TH2F *entriespt0 = new TH2F("entriespt0", "# of entries vs pt[0]; pt[0]; # of entries", 500, 0, 500, 15, 0, 15);
	...
	TH1F *balance = new TH1F("balance", "Dijet Balance; Dijet Balance; Event Fraction", 1000, -3, 3); //REGULAR BINNING
	TH1F *outereta = new TH1F("outereta", "Dijet Balance; Dijet Balance; Event Fraction", 1000, -3, 3); //REGULAR BINNING
	TH1F *innereta = new TH1F("innereta", "Dijet Balance; Dijet Balance; Event Fraction", 1000, -3, 3); //REGULAR BINNING
  //fParent = 0;
  ...
  cout << nEvents << endl; 
    ...
    if (filecounter == 0) {
      if (j_pt[0] > 50) myjtpt = j_pt[0]; //FILLING LEADING JET MOMENTUM VARIABLE   40 TO 60 PT AVERAGE
    }
    if (filecounter == 1) {
      if (j_pt[0] > 80) myjtpt = j_pt[0]; //FILLING LEADING JET MOMENTUM VARIABLE   60 TO 80 PT AVERAGE
    }
    if (filecounter == 2) {
      if(j_pt[0] > 120) myjtpt = j_pt[0]; //FILLING LEADING JET MOMEMTUM VARIABLE   80 TO 140 PT AVERAGE
    }
    if (filecounter == 3) {
      if(j_pt[0] > 170) myjtpt = j_pt[0]; //FILLING LEADING JET MOMEMTUM VARIABLE   140 TO 200 PT AVERAGE
    }
