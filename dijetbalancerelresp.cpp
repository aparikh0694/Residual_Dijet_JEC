...
void dijetbalancerelresp()	{
  ...
  for (int filecounter = 0; filecounter < 5; filecounter++) { //FILE LOOP	
    ...
    if (filecounter == 0) {
      ...
      if (pt->IsOpen()) pt->ls();
      TFile *outf = new TFile("outputrelresp1234.root", "recreate"); // 40 TO 60 PT AVERAGE
      cout << "File 1 Opened Successfully" << endl;
    }
    if (filecounter == 1) {
      TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JSon_Jet40Jet60_ppTrack_forestv84.root"); // 40 TO 80 PT AVERAGE
      if (pt->IsOpen()) pt->ls();
      TFile *outf = new TFile("outputrelresp123.root", "recreate"); // 60 TO 80 PT AVERAGE
    ...
    if (filecounter == 2) {
      ...
      if (pt->IsOpen()) pt->ls();
      TFile *outf = new TFile("outputrelresp1.root", "recreate"); // 80 TO 100 PT AVERAGE
      cout << "File 3 Opened Successfully" << endl;
    }
    if (filecounter == 3) {
      TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JsonPP_Jet80_PPReco_forestv82.root"); // 80 TO 200 PT AVERAGE
      if (pt->IsOpen()) pt->ls();
      TFile *outf = new TFile("outputrelresp.root", "recreate"); // 100 TO 140 PT AVERAGE
      cout << "File 4 Opened Successfully" << endl;
    }
    if (filecounter == 4) {
      TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JsonPP_Jet80_PPReco_forestv82.root"); // 80 TO 200 PT AVERAGE
      if (pt->IsOpen()) pt->ls();
      TFile *outf = new TFile("outputrelresp12.root", "recreate"); // 140 TO 200 PT AVERAGE
