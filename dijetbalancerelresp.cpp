...
void dijetbalancerelresp()	{
  ...
  for (int radius_counter = 1; radius_counter <= 7; radius_counter++) { //RADIUS LOOP
	...
	TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JSon_Jet40Jet60_ppTrack_forestv84.root"); // 40 TO 80 PT AVERAGE
        if (pt->IsOpen()) pt->ls();
	TFile *outf = new TFile(Form("ak%iPFJetAnalyzer/outputrelresp1234.root",radius_counter), "recreate"); // 40 TO 60 PT AVERAGE
        cout << "File 1 Opened Successfully" << endl;
      }
      if (filecounter == 1) {
        TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JSon_Jet40Jet60_ppTrack_forestv84.root"); // 40 TO 80 PT AVERAGE
        if (pt->IsOpen()) pt->ls();
        TFile *outf = new TFile(Form("ak%iPFJetAnalyzer/outputrelresp123.root",radius_counter), "recreate"); // 60 TO 80 PT AVERAGE
      ...
      if (filecounter == 2) {
        ...
        if (pt->IsOpen()) pt->ls();
        TFile *outf = new TFile(Form("ak%iPFJetAnalyzer/outputrelresp1.root",radius_counter), "recreate"); // 80 TO 100 PT AVERAGE
        cout << "File 3 Opened Successfully" << endl;
      }
      if (filecounter == 3) {
        TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JsonPP_Jet80_PPReco_forestv82.root"); // 80 TO 200 PT AVERAGE
        if (pt->IsOpen()) pt->ls();
        TFile *outf = new TFile(Form("ak%iPFJetAnalyzer/outputrelresp.root",radius_counter), "recreate"); // 100 TO 140 PT AVERAGE
        cout << "File 4 Opened Successfully" << endl;
      }
      if (filecounter == 4) {
        TFile *pt = TFile::Open("/afs/cern.ch/work/a/aparikh/public/PP2013_HiForest_PromptReco_JsonPP_Jet80_PPReco_forestv82.root"); // 80 TO 200 PT AVERAGE
        if (pt->IsOpen()) pt->ls();
        TFile *outf = new TFile(Form("ak%iPFJetAnalyzer/outputrelresp12.root",radius_counter), "recreate"); // 140 TO 200 PT AVERAGE
      ...
      TTree* skimt = (TTree *)pt->Get("skimanalysis/HltTree");i
