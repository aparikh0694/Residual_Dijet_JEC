...
void overlayingmc() {
  ...
  for (int radius_counter = 0; radius_counter < number_of_radii; radius_counter++) { //RADIUS LOOP
    ...
    for (int filecounter = 0; filecounter < 5; filecounter++) { //FILE LOOP
      if (filecounter == 0) {
	TFile *montecarlo = new TFile(Form("ak%iPFJetAnalyzer/rebinoutmc1234.root",radius), "Read"); // 40 TO 60 PT AVERAGE
	...
	TFile *ppdata = new TFile(Form("ak%iPFJetAnalyzer/rebinout1234.root",radius), "Read"); // 40 TO 60 PT AVERAGE
	...
	TFile *myfile = new TFile(Form("ak%iPFJetAnalyzer/overlay1234.root",radius), "Recreate"); // 40 TO 60 PT AVERAGE
      }
      if (filecounter == 1) {
	TFile *montecarlo = new TFile(Form("ak%iPFJetAnalyzer/rebinoutmc123.root",radius), "Read"); // 60 TO 80 PT AVERAGE
	if (montecarlo->IsOpen()) printf("MonteCarlo File Opened Successfully.\n");
	montecarlo->ls();
	TGraph *balancemc = (TGraph *)montecarlo->Get("Graph;1");
	TGraph *innermc = (TGraph *)montecarlo->Get("Graph;2");
	TGraph *outermc = (TGraph *)montecarlo->Get("Graph;3");
	TFile *ppdata = new TFile(Form("ak%iPFJetAnalyzer/rebinout123.root",radius), "Read"); // 60 TO 80 PT AVERAGE
	if (ppdata->IsOpen()) printf ("PP Data File Opened Successfully.\n");
	ppdata->ls();
	TGraph *balancedata = (TGraph *)ppdata->Get("Graph;1");
	TGraph *innerdata = (TGraph *)ppdata->Get("Graph;2");
	TGraph *outerdata = (TGraph *)ppdata->Get("Graph;3");
	TFile *myfile = new TFile(Form("ak%iPFJetAnalyzer/overlay123.root",radius), "Recreate"); // 60 TO 80 PT AVERAGE
      }
      if (filecounter == 2) {
	TFile *montecarlo = new TFile(Form("ak%iPFJetAnalyzer/rebinoutmc1.root",radius), "Read"); // 80 TO 100 PT AVERAGE
	if (montecarlo->IsOpen()) printf("MonteCarlo File Opened Successfully.\n");
	montecarlo->ls();
	TGraph *balancemc = (TGraph *)montecarlo->Get("Graph;1");
	TGraph *innermc = (TGraph *)montecarlo->Get("Graph;2");
	TGraph *outermc = (TGraph *)montecarlo->Get("Graph;3");
	TFile *ppdata = new TFile(Form("ak%iPFJetAnalyzer/rebinout1.root",radius), "Read"); // 80 TO 100 PT AVERAGE
	if (ppdata->IsOpen()) printf ("PP Data File Opened Successfully.\n");
	ppdata->ls();
	TGraph *balancedata = (TGraph *)ppdata->Get("Graph;1");
	TGraph *innerdata = (TGraph *)ppdata->Get("Graph;2");
	TGraph *outerdata = (TGraph *)ppdata->Get("Graph;3");
	TFile *myfile = new TFile(Form("ak%iPFJetAnalyzer/overlay1.root",radius), "Recreate"); // 80 TO 100 PT AVERAGE
      }
      if (filecounter == 3) {
	TFile *montecarlo = new TFile(Form("ak%iPFJetAnalyzer/rebinoutmc.root",radius), "Read"); // 100 TO 140 PT AVERAGE
	if (montecarlo->IsOpen()) printf("MonteCarlo File Opened Successfully.\n");
	montecarlo->ls();
	TGraph *balancemc = (TGraph *)montecarlo->Get("Graph;1");
	TGraph *innermc = (TGraph *)montecarlo->Get("Graph;2");
	TGraph *outermc = (TGraph *)montecarlo->Get("Graph;3");
	TFile *ppdata = new TFile(Form("ak%iPFJetAnalyzer/rebinout.root",radius), "Read"); // 100 TO 140 PT AVERAGE
	if (ppdata->IsOpen()) printf ("PP Data File Opened Successfully.\n");
	ppdata->ls();
	TGraph *balancedata = (TGraph *)ppdata->Get("Graph;1");
	TGraph *innerdata = (TGraph *)ppdata->Get("Graph;2");
	...
	TGraph *innermc = (TGraph *)montecarlo->Get("Graph;2");
	TGraph *outermc = (TGraph *)montecarlo->Get("Graph;3");
	TFile *ppdata = new TFile(Form("ak%iPFJetAnalyzer/rebinout12.root",radius), "Read"); // 140 TO 200 PT AVERAGE
	if (ppdata->IsOpen()) printf ("PP Data File Opened Successfully.\n");
	ppdata->ls();
	TGraph *balancedata = (TGraph *)ppdata->Get("Graph;1");
	TGraph *innerdata = (TGraph *)ppdata->Get("Graph;2");
	TGraph *outerdata = (TGraph *)ppdata->Get("Graph;3");
	TFile *myfile = new TFile(Form("ak%iPFJetAnalyzer/overlay12.root",radius), "Recreate"); // 140 TO 200 PT AVERAGE
      }
